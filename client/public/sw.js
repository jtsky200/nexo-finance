...
self.addEventListener('sync', (event) => {
  ...
  } else if (event.tag === 'sync-chat-reminders') {
    event.waitUntil(syncChatReminders());
...
async function syncChatReminders() {
  try {
    console.log('[Service Worker] Syncing chat reminders...');
    
    const clients = await self.clients.matchAll();
    clients.forEach((client) => {
      client.postMessage({
        type: 'SYNC_CHAT_REMINDERS',
        timestamp: new Date().toISOString(),
      });
    });
    ...
    console.error('[Service Worker] Error syncing chat reminders:', error);
    return Promise.reject(error);
  }
}
