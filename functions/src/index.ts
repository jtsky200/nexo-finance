...
function isStatusOpen(s: string | undefined | null): boolean { return !!s && STATUS_OPEN.includes(s.toLowerCase()); }
function isStatusPaid(s: string | undefined | null): boolean { return !!s && STATUS_PAID.includes(s.toLowerCase()); }
function isStatusPostponed(s: string | undefined | null): boolean { return !!s && STATUS_POSTPONED.includes(s.toLowerCase()); }
function isStatusCompleted(s: string | undefined | null): boolean { return !!s && STATUS_COMPLETED.includes(s.toLowerCase()); }
function isStatusOpenOrPostponed(s: string | undefined | null): boolean { return isStatusOpen(s) || isStatusPostponed(s); }
  ...
  for (const personDoc of peopleSnapshot.docs) {
    ...
    for (const invoiceDoc of invoicesSnapshot.docs) {
      ...
      bills.push({
        ...
        installmentEndDate: invoiceData.installmentEndDate?.toDate ? invoiceData.installmentEndDate.toDate().toISOString() : null,
        installments: hasInstallmentPlan ? installments.map((inst: any) => ({
          number: inst.number,
          amount: inst.amount,
          dueDate: inst.dueDate?.toDate ? inst.dueDate.toDate().toISOString() : inst.dueDate,
          status: inst.status,
          paidDate: inst.paidDate?.toDate ? inst.paidDate.toDate().toISOString() : inst.paidDate,
...
export const getCalendarEvents = onCall(async (request) => {
  ...
  for (const personDoc of peopleSnapshot.docs) {
    ...
    for (const invoiceDoc of invoicesSnapshot.docs) {
      ...
      // Skip paid invoices
      if (isStatusPaid(invoiceData.status)) continue;
...
function parseShoppingItems(text: string): Array<{ name: string; quantity?: number; unit?: string; category?: string }> {
  ...
  for (let line of lines) {
    ...
    if (/^(einkaufsliste|shopping|liste|datum|total|summe|Ôé¼|chf|\d+[.,]\d{2}$)/i.test(line)) continue;
...
function parseSwissReceipt(text: string): ReceiptData {
  ...
  result.items = result.items.filter((item, index) => {
    ...
    if (/^\d+\s*[x├ù]$/i.test(name)) return false;
    
    // Remove items with very short names (likely OCR errors)
    if (name.length < 3) return false;
    
    // Remove items that look like metadata
    if (/^(CHF|Total|Summe|Subtotal|MwSt|MWST|Netto|Brutto|Zwischensumme|Rundung|Kartenzahlung|Bar|R├╝ckgeld|Gegeben|Erhalten)/i.test(name)) return false;
    ...
    if (/^\d{4}\s+[A-Za-z├ñ├Â├╝├ä├û├£\-\s]+$/i.test(name)) return false; // Postal code + city
    if (/^[A-Za-z├ñ├Â├╝├ä├û├£]+(?:strasse|str\.|weg|platz|gasse)\s*\d*/i.test(name)) return false; // Street names
...
export const createManualBackup = onCall(async (request) => {
  ...
  try {
    ...
    throw new HttpsError('internal', `Backup failed: ${error.message || 'Unknown error'}`);
  }
});

// Backups auflisten
export const listAllBackups = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const restoreFromBackup = onCall(async (request) => {
  ...
  try {
    const result = await restoreBackup(backupId, userId || request.auth.uid);
    return result;
  } catch (error: any) {
    console.error('[Backup] Error restoring backup:', error);
    throw new HttpsError('internal', `Restore failed: ${error.message || 'Unknown error'}`);
  }
});
...
export const checkReminderNotifications = onSchedule({
...
async function createChatReminder(
  userId: string,
  reminderId: string,
  reminder: any,
...
export const createQuickReminder = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const fixReminderTimes = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
  const userId = request.auth.uid;
  ...
  try {
    ...
    throw new HttpsError('internal', `Fehler beim Korrigieren der Erinnerungen: ${error.message}`);
  }
});

export const createFollowUpReminder = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  if (!originalReminderId) {
    throw new HttpsError('invalid-argument', 'originalReminderId is required');
  }
  ...
  if (!originalReminderDoc.exists || originalReminderDoc.data()?.userId !== userId) {
    throw new HttpsError('permission-denied', 'Not authorized to create follow-up for this reminder');
  }
...
export const getChatReminders = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  let query: admin.firestore.Query = db.collection('chatReminders')
    .where('userId', '==', userId)
...
export const markChatReminderAsRead = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  if (!reminderId) {
    throw new HttpsError('invalid-argument', 'reminderId is required');
  }
  ...
  if (!reminderDoc.exists || reminderDoc.data()?.userId !== userId) {
    throw new HttpsError('permission-denied', 'Not authorized to update this reminder');
  }
...
export const getChatThread = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const getChatConversations = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const createChatConversation = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const updateChatConversation = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const deleteChatConversation = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const getWeatherHistory = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'User must be authenticated');
  }
